...
/**
 * 不能加白名单：加了计算机带不动两个spark app同时运行！！！
 */
//
public class OfflineDomainBehaviorPredict8Attr {
...
//    private static Broadcast<Map<String,String>> broadcastWhitelistMap = null;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        if (onLocal) {
            SparkConf conf = new SparkConf().setAppName(Constants.SPARK_APP_NAME_4)
                    .setMaster("local[5]");  //streaming至少需要两个线程：接收数据，处理数据
        ...
        Set<String> topicSet = KafkaConsumerConfig.consumerConfig(kafkaParams,"MyGroupId3");
        ...
        dnsActionPredict(dnsFlowDF,onLocal,broadcastBlacklistKey,null);
        jssc.start();
...
//        //过滤白名单
//        JavaDStream<ConsumerRecord<String, String>> filterWhiteListDS = dnsFlowDF.filter(r -> {
...
//                    qType+\t+qr+\t+time+\t+rdataLength+\tmainDamain+\t+andataLength
                    ...
                    StringBuilder sb = new StringBuilder("1 ");  //0  包个数 1
                    switch (msg[4]){  //1-2
                        case A: sb.append("1 0 ");break;
                        case AAAA:sb.append("0 1 ");break;
                        default:sb.append("0 0 ");break;
                    }
                    sb.append(msg[4]+" "); //查询类型  //3
                    if(msg[5].equals("1")){  //应答
                        sb.append(msg[9]+" ").append("0 1 ");  //4 5 6 //应答长
                    }else{
                        sb.append("0 ").append("1 0 ");
                    }
                    sb.append(DomainUtil.getHostName(msg[1],msg[8]));  //7
        ...
        JavaPairDStream<String, String> reducedPairDF = filteredPairDF.reduceByKeyAndWindow(new Function2<String, String, String>() {
            ...
            public String call(String f1, String f2) throws Exception {
                String[] a = f1.split(" ");
                //包个数 A个数 AAAA个数 qType anDataLen query个数 response个数 主机名
                //eg: 1 1 0 16 0 1 0 ba8Lb8C4c1bJzTRrCJxAefdUo582q.
                String[] b = f2.split(" ");
                StringBuilder sb = new StringBuilder();
                sb.append(DomainUtil.plus(a[0], b[0]) + " ");//包个数相加 0
                sb.append(DomainUtil.plus(a[1], b[1]) + " "); //A类型相加 1
                sb.append(DomainUtil.plus(a[2], b[2]) + " "); //AAAA类型相加 2
                HashSet<String> set = new HashSet<>();
                set.addAll(Arrays.asList(a[3].split(",")));
                set.addAll(Arrays.asList(b[3].split(",")));  //查询类型 3
                for(String s :set){
                    sb.append(s+",");
                }
                sb = new StringBuilder(sb.substring(0,sb.length()-1)+" "); //去除最后的,

                sb.append(DomainUtil.plus(a[4], b[4]) + " "); //anDataLen类型相加 4
                sb.append(DomainUtil.plus(a[5], b[5]) + " "); //query个数相加 5
                sb.append(DomainUtil.plus(a[6], b[6]) + " "); //reponse个数相加 6
                set.clear();
                set.addAll(Arrays.asList(a[7].split(",")));  //不同host个数 7
                set.addAll(Arrays.asList(b[7].split(",")));
                for(String s :set){
                    sb.append(s+",");
                }
        ...
        JavaDStream<Tuple2<String,Vector>> mainDomain2Features = reducedPairDF.map(t -> {
            ...
            double[] features = DomainUtil.getActionFeatures8Attr(featurePres, windowSize);
        ...
        mainDomain2Features.foreachRDD(rdd->{
            rdd.foreach(v->{
                ...
                "朴素贝叶斯预测结果："+p4);
                //决策树预测树的能预测90%的正常，100%的异常检测
                if(p2>0){
                    //顶级域名存入redis和数据库的黑名单
                    JedisUtil.hset(blacklistKey,v._1,"true");  //1表示异常
                }
